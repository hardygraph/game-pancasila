document.addEventListener('DOMContentLoaded', () => {
    ...
    let timeRemaining = 60; // Set time limit to 60 seconds
    ...
    document.getElementById('check-sentence').addEventListener('click', checkSentences);
    ...
    const timerInterval = setInterval(() => {
        ...
        if (timeRemaining === 0) {
            clearInterval(timerInterval);
            result.textContent = "Waktu habis! Anda belum berhasil menyusun semua kalimat dengan benar.";
            result.style.color = "red";
            document.getElementById('check-sentence').disabled = true;
        }
    }, 1000);
});
